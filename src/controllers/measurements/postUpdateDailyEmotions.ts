...
const postUpdateDailyEmotions = async (req: ExtendedRequest, res: Response) => {
	...
	try {
		...
		const isEmotionExist = await EmotionsConfig.findOne({ _id: emotionsID }, { _id: true }).lean();
		if (!isEmotionExist) {
		...
		logger.info(`Updating daily emotions for user ${usersID}. Data: ${JSON.stringify(req.body)}`);
		...
		logger.error(`Error at controllers/measurements/postUpdateDailyEmotions: ${e}`, e);
