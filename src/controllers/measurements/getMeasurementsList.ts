...
const getList = async (req: ExtendedRequest, res: Response) => {
  ...
  try {
    ...
    const [usersEmotions, usersDailyNotes, usersDailyWeather, emotionsConfig] =
      await Promise.all([
    ...
    const data = datesArray.map((date) => {
      ...
      const usersEmotionsWithConfig = usersEmotions.map((item) => {
        const emotionConfig = emotionsConfig.find(
          (config) => config._id.toString() === item.emotionsID.toString(),
        );
        return {
          date: item.date,
          ...emotionConfig,
      ...
      const usersEmotionByDate = usersEmotionsWithConfig.find(
        (item) => item.date.toISOString() === date.toISOString(),
      );
      const usersDailyNoteByDate = usersDailyNotes.find(
        (item) => item.date.toISOString() === date.toISOString(),
      );
      const usersDailyWeatherByDate = usersDailyWeather.find(
        (item) => item.date.toISOString() === date.toISOString(),
      );
