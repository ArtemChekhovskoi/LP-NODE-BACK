...
const calculateAverageByDate = (
  data: IPatternsListResponseData[],
  ...
  const calculateAverage = (measurements: number[]) => {
    const totalValue = measurements.reduce(
  ...
  data.forEach((item) => {
    ...
    item.measurements.forEach((measurement) => {
      ...
      const uniqueDate = dayjs(measurement.date).format(
  ...
  const result = Array.from(averagesByDate.entries()).map(
    ([uniqueDate, measurements]) => ({
      date: uniqueDate,
  ...
  return data.map((item) => ({
    ...
    measurements: resultWithAverageValues.map((period) => {
      const measurement = period.measurements.find(
        (measurementInfo) => measurementInfo.measurementType === item.code,
      );
      return {
        date: period.date,
