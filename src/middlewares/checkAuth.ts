...
const checkAuth = async (
  ...
  const isNotProtectedRoute = config.auth.notProtectedRoutes.find((route) =>
  ...
  try {
    if (
      ...
      const jwtData: JWTData | string = jwt.verify(
        token,
      ...
      const isSessionIDValid =
        jwtData &&
        typeof jwtData.sessionID === "string" &&
        validator.isMongoId(jwtData.sessionID);
      ...
      if (!usersID) {
        throw new Error(
          `Not found usersID - in session ${session._id}, token: ${token}`,
        );
      }
      ...
      if (!user) {
        throw new Error(
          `User not found - by session ${session._id}, usersID: ${usersID}, token: ${token}`,
        );
      }
